...
`, snapshotName, testNs, volumeSnapshotClassName, pvcOriName)
	...
	g.It("Author:mmakwana-High-88799-[OTP][LVMS] Verify reclaimPolicy and volumeBindingMode combinations", g.Label("SNO", "MNO"), func() {
		...
		o.Eventually(func() string {
			out, _ := exec.Command("oc", "get", "pvc", pvcDeleteImmediate, "-n", testNs,
		...
		o.Eventually(func() bool {
			out, _ := exec.Command("oc", "get", "pv", pvDeleteImmediate, "--ignore-not-found", "-o", "jsonpath={.metadata.name}").Output()
			return strings.TrimSpace(string(out)) == ""
		}, ResourceDeleteTimeout, 5*time.Second).Should(o.BeTrue())
		...
		g.By("#. Verify PVC is in Pending state (WaitForFirstConsumer requires a consumer)")
		o.Eventually(func() string {
			out, _ := exec.Command("oc", "get", "pvc", pvcRetainWFFC, "-n", testNs,
				"-o", "jsonpath={.status.phase}").Output()
			return strings.TrimSpace(string(out))
		...
		o.Eventually(func() bool {
			out, _ := exec.Command("oc", "get", "pvc", pvcRetainWFFC, "-n", testNs, "--ignore-not-found", "-o", "jsonpath={.metadata.name}").Output()
			return strings.TrimSpace(string(out)) == ""
		}, ResourceDeleteTimeout, 5*time.Second).Should(o.BeTrue())

		g.By("#. Verify PV exists with status Released (reclaimPolicy=Retain)")
		o.Eventually(func() string {
			out, _ := exec.Command("oc", "get", "pv", pvRetainWFFC, "-o", "jsonpath={.status.phase}").Output()
			return strings.TrimSpace(string(out))
		...
		o.Eventually(func() bool {
			out, _ := exec.Command("oc", "get", "pv", pvRetainWFFC, "--ignore-not-found", "-o", "jsonpath={.metadata.name}").Output()
			return strings.TrimSpace(string(out)) == ""
		}, ResourceDeleteTimeout, 5*time.Second).Should(o.BeTrue())
